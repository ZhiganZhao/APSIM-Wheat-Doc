...
$$
BD_{stem,\,storage} = [\frac{B_{stem,\,structural} + BD_{stem,\,structural}}{F_{stem, structure}} - (B_{stem} + BD_{stem,\,structural})] / C_{stem, efficiency}
$$
...
Stem nitrogen is only allocated into two components (i.e. structural and storage) as the same minimum and critical nitrogen concentrations (Figs \@ref(fig:stem-nitrogen-diagram-dynamic) and \@ref(fig:stem-nitrogen-cardinal)). The nitrogen in the storage can be retranslocated into other organs as a daily fraction (Section \@ref(sec:stem-nitrogen-supply)). No reallocation of nitrogen occurs in stem as no senescing and dead pool of biomass and nitrogen.
...
The minimum and critical nitrogen concentrations keep constant during whole season, but the maximum nitrogen concentrations graudally reducing from `Emergence` until `Flowering` (Fig. \@ref(fig:stem-nitrogen-cardinal)).
...
No reallocation of nitrogen occurs from the senescence stem as the senescence rate is set as `r stem_senesce_rate` for stem (Fig. \@ref(fig:stem-nitrogen-supply)). No reallocation of nitrogen occurs from metabolic component as no metabolic is simulated in stem (Fig. \@ref(fig:stem-dm-weight)). No fixation and uptake nitrogens are simulated in stem (Fig. \@ref(fig:stem-nitrogen-supply)).
...
Structural nitrogen is used to satisfy minimum concentration, metabolic to fit critical concentration, storage to fit maximum concentration. 



<div class="fig-input">
```{r stem-nitrogen-para-conc, fig.asp = 1, fig.cap='Minimum, critical and maximum nitrogen concentration in stem. Stem has the same minimum and critical nitrogen concentration.'}
y_cols <- c('Wheat.Stem.MinimumNConc',
'Wheat.Stem.CriticalNConc',
'Wheat.Stem.MaximumNConc')
y_labels <- c('Minimum', 'Critical', 'Maximum')
...
<div class="fig-output">
```{r stem-nitrogen-wt, fig.asp = 1, fig.cap='Dynamic of stem nitrogen for three components and total.'}
y_cols <- c(sprintf('Wheat.Stem.%sN', g_demand), 'Wheat.Stem.N')
...
<div class="fig-output">
```{r stem-nitrogen-live, fig.asp = 1, fig.cap='Dynamic of stem nitrogen in Live pool for three components and total.'}
y_cols <- c(sprintf('Wheat.Stem.Live.%sN', g_demand), 'Wheat.Stem.Live.N')
...
<div class="fig-output">
```{r stem-nitrogen-dead, fig.asp = 0.5, fig.cap='Dynamic of stem nitrogen in Dead pool for three components and total.'}
y_cols <- c(sprintf('Wheat.Stem.Dead.%sN', g_demand), 'Wheat.Stem.Dead.N')
