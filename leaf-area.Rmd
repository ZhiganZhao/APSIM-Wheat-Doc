...
TT <- seq(0, 100, by = 1)
MaxArea <- 2600
LeafSizeShape <- as.numeric(get_fixed_value(pmf, 'Wheat.Leaf.CohortParameters.LeafSizeShapeParameter'))
GrowthDuration <- 100
...
The extinction coefficient for dead leaf ($k_{d}$) is defined as `r get_xml_value(pmf, '//Leaf/KDead')`. The extinction coefficient for green leaf ($k_{g}$) is calculated by parameter `ExtinctionCoeff` with default value `r get_fixed_value(pmf, 'Wheat.Leaf.ExtinctionCoeff')`.  
