...
Wheat plants are separated into `r length(g_organs)` organs, (i.e. `r paste(g_organs, collapse = ', ')`). Leaf includes only leaf blades. Stem is defined in a functional rather than a morphological manner and includes nodes, internodes and leaf sheaths. Spike is the head parts without grain including pedicel and glunes.
...
```{r biomass-supply-fig-infor, results='asis'}
c("Figure,Reference",
    "Total supply by components,Fig. \\@ref(fig:biomass-supply-total)",
    "Total supply by organs,Fig. \\@ref(fig:biomass-supply-organ)",
    "Fixation by organs,Fig. \\@ref(fig:biomass-supply-fixation)",
    "Retranslocation by organs,Fig. \\@ref(fig:biomass-supply-retranslocation)",
    "Reallocation by organs,Fig. \\@ref(fig:biomass-supply-reallocation)") %>%
    paste(collapse = "\r\n") %>% 
    textConnection() %>% 
    read.csv() %>% 
    knitr::kable(align = "lc",
                 caption = 'Figures about biomass supply.')
```

<div class="fig-output">
```{r biomass-supply-total, fig.asp = 1, fig.cap='Biomass total supply by components'}
...
<div class="fig-output">
```{r biomass-supply-organ, fig.asp = 1, fig.cap='Biomass total supply by organs'}
...
<div class="fig-output">
```{r biomass-supply-reallocation, fig.asp = 0.5, fig.cap='Biomass reallocation from all organs'}
y_cols <- c(sprintf('Wheat.%s.DMSupply.Reallocation', g_organs),
           "Wheat.DMSupply.Reallocation")
plot_report(g_report, g_xvar2, y_cols, x_lab = g_xlab2, 
            y_lab =  expression('rReallocation (g biomass'~m^-2~" ground"~d^-1~')'), 
            y_labels = c(g_organs, "Total"),
            ncol = 6)
```
</div>
...
plot_report(g_report, g_xvar, y_cols, x_lab = g_xlab, 
            y_lab = 'Structural and metabolic demand (g/m2/d)', y_labels = g_organs,
            ncol = 5)
```
</div>
...
plot_report(g_report, g_xvar, y_cols, x_lab = g_xlab, 
            y_lab = 'Structural demand (g/m2/d)', y_labels = g_organs,
            ncol = 5)
...
plot_report(g_report, g_xvar, y_cols, x_lab = g_xlab, 
            y_lab = 'Storage demand (g/m2/d)', y_labels = g_organs,
            ncol = 5)
