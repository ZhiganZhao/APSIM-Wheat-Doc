...
where $I_{0}$ is the total radiation at the top of the canopy (MJ) which is directly imported from weather records. Extinction coefficient ($k$) set as a constant value  `r get_fixed_value(pmf, 'Wheat.Leaf.ExtinctionCoeff')`.  
...
**The potential RUE ($RUE_p$)** has a default value `r get_fixed_value(pmf, paste0(name_rue, '.RUE'))`.
...
plot_xypair(pmf, path, x_lab = 'Average daily temperature weighted toward maximum temperature', y_lab = 'Temperature factor to photosynthesis')
```
</div>
...
plot_xypair(pmf, path, x_lab = 'Ratio of functional nitrogen in leaf.',
...
plot_xypair(pmf, path, x_lab = 'Water stress',
