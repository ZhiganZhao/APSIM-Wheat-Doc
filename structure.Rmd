...
The base phyllochron is a genotypic parameter with default value `r get_fixed_value(pmf, 'Wheat.Structure.Phyllochron.BasePhyllochron')`, but is chagned for most of cultivars. Based on [@JamiesonPredictionleafappearance1995], leaf appearance could be described by a base phyllochron determined between leaves 3 and 7 and a phyllochron that was 70% of base phyllochron for leaves < 3 and 140% of base phyllochroen for leaves > 7 (Fig. \@ref(fig:str-phyllochron-age)). The phyllochron also is adjusted by photoperiod [reference required] through increasing pholochron in the shorter day length (Fig. \@ref(fig:str-phyllochron-ppd)).


<div class="fig-input">
```{r str-phyllochron-age, fig.asp = 0.5, fig.cap='Phyllochron of leaf cohort is depending on the rank on the main stem'}
xpath <- 'Wheat.Structure.Phyllochron.LeafStageFactor'
...
xpath <- 'Wheat.Structure.Phyllochron.PhotoPeriodEffect'
...
xpath <- 'Wheat.Structure.BranchingRate.PotentialBranchingRate.Vegetative.PotentialBranchingRate'
...
xpath <- paste0('Wheat.Structure.BranchingRate.StressFactors.',
...
At the terminal spikelet, all tillers with less than `r get_fixed_value(pmf, 'Wheat.Structure.StemSenescenceAge')` leaves are stopped to growth new leaves.
...
xpath <- 'Wheat.Structure.BranchMortality.MortalityPhase.Mortality.MortalityPerDegDay'
...
xpath <- 'Wheat.Structure.HeightModel.PotentialHeight'
...
xpath <- 'Wheat.Structure.HeightModel.WaterStress'
